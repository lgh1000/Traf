...
//from very large city with intense traffic, cars: 432; TrafficCar lights: 67. 
//Total observations : no fo carsx2 = 864 observations from cars + 67traffic lights x 3(138)= 201; Total: 1065 observations
//    Continuous actions are 432 cars +67 lights = total 499
...
public class ControllerAgent : Agent
{
    ...
    {
        base.Awake(); // Call the base class Awake method first
    ...
    public override void CollectObservations(VectorSensor sensor)
    {
        ...
        // Original model expects 1896 observations
        // For traffic lights: 46 lights * 3 observations = 138
        // For cars: (1896 - 138) / 2 = 879 cars with 2 observations each
        int targetLightCount = 46;  // Original number of lights
        int targetCarCount = 879;   // Original number of cars
        ...
        // Take only the first targetCarCount cars
        int carsToProcess = Mathf.Min(activeCarIndices.Count, targetCarCount);
        for (int i = 0; i < carsToProcess; i++)
        {
        ...
        // Pad remaining car observations if we have fewer cars than target
        int remainingCarObs = (targetCarCount - carsToProcess) * 2;
        for (int i = 0; i < remainingCarObs; i++)
        {
            sensor.AddObservation(0f);
        }

        // Process only targetLightCount lights
        int lightsToProcess = Mathf.Min(trafficLights.Length, targetLightCount);
        for (int i = 0; i < lightsToProcess; i++)
        {
            var light = trafficLights[i];
            sensor.AddObservation(light.transform.localPosition.z);
            sensor.AddObservation(light.transform.localPosition.x);
            sensor.AddObservation(light.step1Duration);
        }

        // Pad remaining light observations if we have fewer lights than target
        int remainingLightObs = (targetLightCount - lightsToProcess) * 3;
        for (int i = 0; i < remainingLightObs; i++)
        {
            sensor.AddObservation(0f);
        }

        //Debug.Log($"Cars processed: {carsToProcess}/{targetCarCount}, Lights processed: {lightsToProcess}/{targetLightCount}");
    }
    ...
    public override void OnActionReceived(ActionBuffers actions)//CHANGE/ UNCOMMENT BACK! continuous actions output as -1 to 1.
    {
        ...
        // Original model expects 46 lights and 879 cars
        int targetLightCount = 46;
        int actualLightCount = Mathf.Min(trafficLights.Length, targetLightCount);

        // Modify your action processing to only use available actions
        //var maxActions = System.Math.Min(actions.ContinuousActions.Length, trafficCars.Length + trafficLights.Length);

        // Process only the actions we can use
        for (int i = 0; i < actions.ContinuousActions.Length; i++)
        {
            if (i < actualLightCount)
            {
                lightActions.Add(actions.ContinuousActions[i]);
            }
            else if (i < targetLightCount)
            {
                // Skip extra light actions
                continue;
            }
            else if (i - targetLightCount < trafficCars.Length)
            {
